...
async def play_hndlr(
    ...
    if not file.file_path:
        # Prefer remote CDN streaming so the bot starts the song without
        # first downloading the complete file. Video mode keeps the existing
        # download path because the direct API stream is audio-oriented.
        if not video:
            try:
                file.file_path = await yt.stream_url(file.id, video=False)
            except Exception:
                file.file_path = None

        # Reliable fallback: use the existing downloader if direct streaming
        # is unavailable.
        if not file.file_path:
    ...
    if not file.file_path:
        return await sent.edit_text(
            "❌ Unable to get a playable stream. Please try another song."
        )
